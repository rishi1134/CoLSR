...
class DebugTransform:
    ...
    def __call__(self, image, target):
        ...
        image_after, target_after = self.transform(image, target)
        
        if(target_after['boxes'].shape[0] == 0):
            return image, target
        
        return image_after, target_after
...
def make_coco_transforms(image_set, fix_size=False, strong_aug=False, args=None):
    ...
    if image_set == 'train':
        ...
        return T.Compose([
            DebugTransform(T.RandomHorizontalFlip(), "RandomHorizontalFlip"),
            T.RandomResize(scales, max_size=max_size),
            normalize,
        ])
