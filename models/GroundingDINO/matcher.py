...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, focal_alpha = 0.25, cost_neigh: float = 1):
        """Creates the matcher
        Params:
        ...
        self.cost_neigh = cost_neigh
        assert cost_class != 0 or cost_bbox != 0 or cost_giou != 0 or cost_neigh != 0, "all costs cant be 0"
    ...
    def forward(self, outputs, targets, label_map):
        ...
        cost_bbox = torch.cdist(out_bbox[:, :2], tgt_bbox[:, :2], p=1) # 900, 56
...
def build_matcher(args):
    ...
    if args.matcher_type == 'HungarianMatcher':
        return HungarianMatcher(
            ...
            focal_alpha=args.focal_alpha, cost_neigh=args.set_cost_neigh
        )
